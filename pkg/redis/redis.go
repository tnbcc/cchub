...
func (rds RedisClient) Set(key string, value interface{}, expiration time.Duration) bool {
	if err := rds.Client.Set(rds.Context, key, value, expiration).Err(); err != nil {
		log.ErrorString("Redis", "Set", err.Error())
		return false
	}
	return true
}
...
func (rds RedisClient) Get(key string) string {
	...
	if err != nil {
		if err != redis.Nil {
			log.ErrorString("Redis", "Get", err.Error())
		}
		return ""
	}
	return result
...
func (rds RedisClient) Has(key string) bool {
	...
	if err != nil {
		if err != redis.Nil {
			log.ErrorString("Redis", "Has", err.Error())
		}
		return false
	}
	return true
...
func (rds RedisClient) Del(keys ...string) bool {
	if err := rds.Client.Del(rds.Context, keys...).Err(); err != nil {
		log.ErrorString("Redis", "Del", err.Error())
		return false
	}
	return true
}
...
func (rds RedisClient) FlushDB() bool {
	if err := rds.Client.FlushDB(rds.Context).Err(); err != nil {
		log.ErrorString("Redis", "FlushDB", err.Error())
		return false
	}
	return true
}
...
func (rds RedisClient) Increment(parameters ...interface{}) bool {
	...
	case 1:
		...
		if err := rds.Client.Incr(rds.Context, key).Err(); err != nil {
			log.ErrorString("Redis", "Increment", err.Error())
			return false
		}
	case 2:
		key := parameters[0].(string)
	...
	default:
		log.ErrorString("Redis", "Increment", "参数过多")
		return false
	}
	return true
}
...
func (rds RedisClient) Decrement(parameters ...interface{}) bool {
	...
	case 1:
		...
		if err := rds.Client.Decr(rds.Context, key).Err(); err != nil {
			log.ErrorString("Redis", "Decrement", err.Error())
			return false
		}
	case 2:
		key := parameters[0].(string)
	...
	default:
		log.ErrorString("Redis", "Decrement", "参数过多")
		return false
	}
	return true
}
